...
<!--
  ...
  仓库点开率和一段纯文字完全不是一个量级。
  <div align="center"><img src="C:/Users/cao/Downloads/langgraph-ai-rd-agent-main/docs/screenshots/chat.png" width="800" alt="问答界面" /></div>
-->
...
本地开发只需要两个东西：一个 LLM API Key，和一个本地 Embedding 模型。
...
**完整 VS Code 流程**（解释器选择、依赖精简安装、断点位置、故障排查）见
📄 [`VSCODE_SETUP.md`](VSCODE_SETUP.md)。

**依赖安装建议**：Windows / CPU 环境优先用 `requirements-lite.txt`（约 1 GB）。
`requirements.txt` 会连带拉入 CUDA 版 torch（约 2.5 GB），而本服务的嵌入推理始终跑在 CPU 上。
